...
void solve(){
    ...
    vi BIT(1e6+10),Y(1e6+10); // 记录出现次数
    auto add_bit = [&](int x,int v){
        for(int i=x;i<1e5+10;i += i&-i){
            BIT[i] += v;
        }
    };
    auto get_bit = [&](int x){
    ...
    for(int k=0,i=0,j=1;k<m;++k){
        ...
        while(j<x0) del(j++);
        while(j>x0) add(--j);
        ans[id] = get_bit(y1)-get_bit(y0-1);
    }
